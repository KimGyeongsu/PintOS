  ...
  for (;;)
    {
      ...
      if (type == USTAR_EOF)
      ...
      else if (type == USTAR_REGULAR)
        {
         ...
         //printf("file_name %s and size %d\n",file_name,size);
          //struct parsing_result *result = parse(file_name, true);


          if (!filesys_create (file_name, size)) //argsnone : 52764
            PANIC ("%s: create failed", file_name);
          //printf("2222\n");

//printf("ITS NAME is %s and initial_sizfe %d\n",file_name,size);
  /*printf("Before do it, free_map first flase index is %d\n",
       bitmap_scan(free_map, 0,1,false));
  struct inode_disk *inode_disk = malloc(sizeof(struct inode_disk));
  block_read(fs_device, 0, inode_disk);
  printf("This is free_map_inode_disk(0) : ");
  ...
  printf("Information : length %d, direct_blocks[0] %d direct_blocks[1] %d\n",
            inode_disk->length, inode_disk->direct_blocks[0],inode_disk->direct_blocks[1]);
  printf("Free_map loading from free_map_file(2)\n");
  bitmap_write(free_map, free_map_file);
  printf("Now free_map first flase index is %d\n",
       bitmap_scan(free_map, 0,1,false));

  printf("Let's look directory_entry(3)\n");
  struct dir_entry e;
  off_t ofs;
  //printf("")
  for (ofs = 0; inode_read_at (inode_open(3), &e, sizeof e, ofs) == sizeof e;
       ofs += sizeof e){
    printf("inode_sector %d, name %s, in_use %d\n",e.inode_sector, e.name,e.in_use); 
  }

  printf("This is args-none-inode_disk(4) : ");
  block_read(fs_device, 4, inode_disk);
  printf("Information : length %d, direct_blocks[0] %d direct_blocks[1] %d ",
            inode_disk->length, inode_disk->direct_blocks[0],inode_disk->direct_blocks[1]);
  printf("direct_blocks[103] %d direct_blocks[104] %d\n",
            inode_disk->direct_blocks[103],inode_disk->direct_blocks[104]);*/
