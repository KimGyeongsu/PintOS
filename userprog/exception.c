...
page_fault (struct intr_frame *f) 
{
  ...
  void *esp;
  if(user) esp = f->esp;
  else esp = thread_current()->saved_esp;

  /*Deal with valid access */
  if(not_present && fault_addr > USER_VADDR_BOTTOM && is_user_vaddr(fault_addr)){
    sema_down(&thread_current()->spt_sema);

    struct spte *spte = get_spte(thread_current()->spt, pg_round_down(fault_addr));
    if(spte!=NULL){
      //printf("spte_is_existed\n");
      //printf("pagefault: spte is %x\n", spte->upage);
      ...
      else if(spte->state==MEMORY || spte->state == MMAP_MEMORY) {
        PANIC ("page fault occurs wrong"); 
      }
    ...
    else if(fault_addr >= esp - STACK_HEURISTIC && MAX_STACK_GROWTH > PHYS_BASE - pg_round_down(fault_addr)) {
      //sema_down(&thread_current()->mte_sema);
      lock_acquire(&swap_lock);
      lock_acquire(&frame_table_lock);
  ...
  if(!is_user_vaddr(fault_addr)||fault_addr==NULL||not_present) exit_handler(-1);
